...
let find_proof_using (ast : Synterp.vernac_control_entry) =
  ...
  | _ -> log.debug (fun () -> "no ast for proof using, looking at a default");
      None
...
let push_state id ast synterp classif st =
  ...
  | VtQed terminator_type ->
    log.debug (fun () -> "scheduling a qed");
    ...
    | block :: pop ->
      ...
      | None ->
        log.debug (fun () -> "not an opaque proof");
...
let schedule_sentence (id, (ast, classif, synterp_st)) st schedule =
  ...
  in
(*
  log.debug (fun () -> "Scheduled " ^ (Stateid.to_string id) ^ " based on " ^ (match base with Some id -> Stateid.to_string id | None -> "no state"));
  log.debug (fun () -> "New scheduler state: " ^ string_of_state st);
  *)
